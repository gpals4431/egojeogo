...
public class SubwayRealTimeApi implements SubwayArrivalPort {
    ...
    public List<SubwayArrival> getSubwayRealTimeArrival(FavoriteStation favoriteStation) {
        try {
            System.out.println("favoriteStation.getStationName() = " + favoriteStation.getStationName());
            // 1. API 호출 → XML 문자열 받기
            ...
            // 3. DTO → 도메인 모델로 변환
            List<SubwayArrival> result = toDomainList(response, favoriteStation.getUpDownLine(), favoriteStation.getLine());
    ...
    private boolean matchesLine(String subwayId, String line) {
        ...
        try {
            ...
            String lineName = subwayLine.getName();
            
            // 숫자 호선 (1~9) → "1호선", "2호선"으로 변환
            if (line.matches("\\d+")) {
                String expectedName = line + "호선";
                return lineName.equals(expectedName);
            }
            
            // 특수 노선 (경의중앙선, 공항철도 등) → 그대로 비교
            return lineName.equals(line);
